...
class DamageSensorDataset:
    ...
    def __init__(self,
        ...
        #### Load data
        ramps = torch.load(ramps_path, weights_only=True)      # shape (n_samples, res_x, res_y)
        damage = torch.load(damage_path, weights_only=True)    # shape (n_samples, res_x, res_y, time_steps)
        defgrad = torch.load(defgrad_path, weights_only=True)  # shape (n_samples, res_x, res_y, time_steps)
        elec = torch.load(elec_path, weights_only=True)        # shape (n_samples, res_x, res_y, time_steps)
        masks = torch.load(masks_path, weights_only=True)      # shape (n_masks,   res_x, res_y)
